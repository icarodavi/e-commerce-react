...
export function Payed() {
    const { cart,contextClearCart } = useContextSelector(ShopContext, context => context);
            ...
            <div className="bg-white">
                ...
                <main className="relative mx-auto grid max-w-7xl grid-cols-1 gap-x-16 lg:grid-cols-2 lg:px-8">
                    ...
                    >
                        <div className="mx-auto max-w-2xl px-4 lg:max-w-none lg:px-0 mt-2">
                            ...
                            <dl>
                                ...
                                <dd className="mt-1 text-3xl font-bold tracking-tight text-white">{payedCart.length > 0 ? priceFormatter.format(Number(sumItem(payedCart))) : '$0.00'}</dd>
                            </dl>

                            <ul role="list" className="divide-y divide-white divide-opacity-10 text-sm font-medium">
                                {payedCart && payedCart.map((product) => (
                            ...
                            <dl className="space-y-6 border-t border-white border-opacity-10 pt-6 text-sm font-medium">
                                <div className="flex items-center justify-between">
                                    <dt>Subtotal</dt>
                                    <dd>{payedCart.length > 0 ? priceFormatter.format(Number(sumItem(payedCart))) : '$0.00'}</dd>
                                </div>
                                ...
                                <div className="flex items-center justify-between border-t border-white border-opacity-10 pt-6 text-white">
                                    ...
                                    <dd className="text-base">{payedCart.length > 0 ? priceFormatter.format(Number(sumItem(payedCart) + 5)) : '$0.00'}</dd>
                                </div>
                            </dl>
                        </div>
                    </section>
                    ...
                    <div className="flex flex-col justify-center items-center">
                        <div className=" text-indigo-800"><AiOutlineCheckCircle className="h-40 w-40" /></div>
                        <p>Success! Your payment has been processed and your order will arrive soon.</p>
                        <UIButton className="mt-4" onClick={() => { navigate('/'); }}><div className="flex flex-row gap-2 justify-center items-center"><AiOutlineHome className="h-5 w-5" />Go Home</div></UIButton>
                    </div>
